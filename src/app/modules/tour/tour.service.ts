...
import { tourSearchableFields, tourTypeSearchableFields } from "./tour.constants";
...
const updateTour = async (id: string, payload: Partial<ITour>) => {
    ...
    if(payload.deleteImages && payload.deleteImages.length > 0  && existingTour.images  && existingTour.images.length > 0){
        ...
        const updatePayloadImages = (payload.images || []).filter(imgUrl => !payload.deleteImages?.includes(imgUrl)).filter(imgUrl => !restDBImages.includes(imgUrl))
